...
def analisis_sensores():
    ...
    if st.button("Cargar datos") or st.session_state.df_trafico is not None:
        if st.session_state.df_trafico is None:
            with st.spinner('Cargando datos...'):
                try:
                    st.session_state.df_trafico = cargar_datos_trafico_y_meteo()
                except Exception as e:
                    st.error(f"Error al cargar los datos: {str(e)}")
                    return

        df = st.session_state.df_trafico
